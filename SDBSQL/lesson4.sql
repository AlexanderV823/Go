...
/*
EXPLAIN
-> Nested loop inner join  (cost=1.86 rows=1)
    -> Index lookup on a using idx_fk_city_id (city_id=17)  (cost=0.95 rows=1)
    -> Index lookup on c using idx_fk_address_id (address_id=a.address_id)  (cost=0.91 rows=1)
EXPLAIN ANALYZE
-> Nested loop inner join  (cost=2.05 rows=1) (actual time=0.947..0.950 rows=1 loops=1)
    -> Index lookup on a using idx_fk_city_id (city_id=17)  (cost=0.95 rows=1) (actual time=0.072..0.073 rows=1 loops=1)
    -> Index lookup on c using idx_fk_address_id (address_id=a.address_id)  (cost=1.10 rows=1) (actual time=0.872..0.874 rows=1 loops=1)
*/
...
/*
-> Filter: (customer.first_name = 'MARY')  
	(cost=61.15 rows=60) (actual time=0.062..0.363 rows=1 loops=1)
    -> Table scan on customer  
    (cost=61.15 rows=599) (actual time=0.060..0.326 rows=599 loops=1)
*/
...
/*
-> Index lookup on customer using idx_last_name 
(last_name='SMITH')  
(cost=0.35 rows=1) (actual time=0.024..0.026 rows=1 loops=1)
*/
EXPLAIN ANALYZE 
SELECT *
FROM customer
...
/*
-> Index range scan on customer using idx_last_name, with index condition: 
...
/*-> Filter: (customer.last_name like '%SMITH')  
 * (cost=61.15 rows=67) (actual time=0.063..0.395 rows=1 loops=1)
    -> Table scan on customer  (cost=61.15 rows=599) 
    (actual time=0.061..0.312 rows=599 loops=1)
*/
...
INSERT INTO film_temp (`title`, `description`, `release_year`, `language_id`)
SELECT 
	f1.`title`, 
	f1.`description`, 
	CONCAT('202', FLOOR(RAND() * 5)),
	FLOOR(RAND() * 5) 
FROM film f1, film f2; 

SELECT COUNT(*) FROM film_temp;

SELECT * FROM information_schema.STATISTICS 
WHERE TABLE_SCHEMA = 'sakila' AND TABLE_NAME = 'film';

SELECT * FROM information_schema.STATISTICS 
WHERE TABLE_SCHEMA = 'sakila' AND TABLE_NAME = 'film_temp';

ALTER TABLE sakila.film_temp MODIFY COLUMN film_id bigint unsigned NOT NULL;
ALTER TABLE sakila.film_temp DROP KEY film_id;
...
/*-> Filter: (film_temp.film_id = 100)  (cost=108918.30 rows=99274) (actual time=0.521..711.323 rows=1 loops=1)
    -> Table scan on film_temp  (cost=108918.30 rows=992743) (actual time=0.321..669.845 rows=1000000 loops=1)
*/

ALTER TABLE film_temp
...
SELECT *
FROM film_temp 
WHERE language_id = 1 AND release_year = 2021;

/*-> Filter: ((film_temp.release_year = 2021) and (film_temp.language_id = 1))  (cost=109721.90 rows=9918) (actual time=0.053..1057.632 rows=39973 loops=1)
    -> Table scan on film_temp  (cost=109721.90 rows=991809) (actual time=0.025..977.626 rows=1000000 loops=1)
*/

-- https://dev.mysql.com/doc/refman/8.0/en/create-index.html
-- составной индекс
CREATE INDEX lang_year_idx ON film_temp (language_id, release_year);
...
/*-> Index lookup on film_temp using lang_year_idx 
 * (language_id=1, release_year=2021)  
 * (cost=39775.40 rows=81524) 
 * (actual time=0.027..674.214 rows=39973 loops=1)*/

DROP INDEX lang_year_idx ON film_temp ;

EXPLAIN ANALYZE
SELECT *
FROM film_temp 
WHERE language_id = 1 AND title = 'WONKA SEA';

/*-> Filter: ((film_temp.title = 'WONKA SEA') and (film_temp.language_id = 1))  
 *(cost=109721.90 rows=9918) 
 *(actual time=1.818..1116.176 rows=222 loops=1)
    -> Table scan on film_temp  
    (cost=109721.90 rows=991809) 
    (actual time=0.354..1012.167 rows=1000000 loops=1)
*/

CREATE INDEX title_lang_idx ON film_temp (title, language_id);
...
/*-> Index lookup on film_temp using title_lang_idx (title='WONKA SEA', language_id=1)
 *   (cost=244.20 rows=222)
 *   (actual time=0.027..1.979 rows=222 loops=1)*/


DROP INDEX title_lang_idx ON film_temp ;
CREATE INDEX lang_title_idx ON film_temp (language_id, title);
...
/*-> Index lookup on film_temp using lang_title_idx (language_id=1, title='WONKA SEA')  
 * (cost=244.20 rows=222) 
 * (actual time=0.412..25.671 rows=222 loops=1)
*/
DROP INDEX title_lang_idx ON film_temp ;

-- фукнциональный индекс
...
EXPLAIN ANALYZE SELECT * FROM tbl WHERE SUBSTRING(col1, 1, 9)='123456789'; 
/*-> Filter: (substr(tbl.col1,1,9) = '123456789')  
 * (cost=0.35 rows=1) 
...
*/
EXPLAIN ANALYZE SELECT * FROM tbl WHERE SUBSTRING(col1, 1, 10)='1234567890';
/*-> Index lookup on tbl using idx1 
 * (substr(col1,1,10)='1234567890')  
 * (cost=0.35 rows=1) 
...
--  Логи медленный запросов
-- писать в лог медленные запросы
-- log-slow-queries=/logs/slow_queries.log 

-- учитывать только запросы, выполняющиеся больше 5 секунд
-- long_query_time=5
-- логировать запросы, не использующие индексы
...
-- (OFF, DEMAND, ON).
-- OFF отключает кэширование,
-- DEMAND – кэширование будет производиться только при наличии директивы SQL_CACHE в запросе,
-- ON включает кэширование.	
-- query_cache_type=0
...
SELECT 
	...
	(SELECT hometown FROM profiles WHERE user_id = 1) AS 'Город',
	(SELECT filename FROM media WHERE id = 
	    (SELECT photo_id FROM profiles WHERE user_id = 1)) AS 'Аватарка'
FROM users
WHERE id=1;
-- всех 
EXPLAIN
...
-- если индекс, то =  и IN, но не LIKE 
