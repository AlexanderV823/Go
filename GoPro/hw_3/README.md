...
| Метод      | Эндпоинт      | Описание                           | Тело запроса (JSON)                 | Код (Успех) |        Код (Ошибка)        | Заголовок Allow (при 405) |
| :--------- | :------------ | :--------------------------------- | :---------------------------------- | :---------: | :------------------------: | :------------------------ |
| **GET**    | `/health`     | Проверка работоспособности сервиса | —                                   |    `200`    |           `405`            | `GET`                     |
| **GET**    | `/tasks`      | Получить список всех задач         | —                                   |    `200`    |           `405`            | `GET, POST`               |
| **POST**   | `/tasks`      | Создать новую задачу               | `{"title": "string"}`               |    `201`    |    `400`, `405`, `500`     | `GET, POST`               |
| **GET**    | `/tasks/{id}` | Получить задачу по идентификатору  | —                                   |    `200`    |    `400`, `404`, `405`     | `GET, PUT, DELETE`        |
| **PUT**    | `/tasks/{id}` | Обновить задачу целиком            | `{"title": "string", "done": bool}` |    `200`    | `400`, `404`, `405`, `500` | `GET, PUT, DELETE`        |
| **DELETE** | `/tasks/{id}` | Удалить задачу по идентификатору   | —                                   |    `204`    | `400`, `404`, `405`, `500` | `GET, PUT, DELETE`        |


Все ответы (включая ошибки), за исключением кода 204, возвращаются с заголовком Content-Type: application/json. Единый формат ошибок: {"error": "сообщение"}.
...
* **Передача метода POST на карточку ресурса**

**Успешный сценарий:**
curl -i -X POST http://localhost:8080/tasks/1
...
Content-Type: application/json
Date: Thu, 06 Aug 2026 16:00:00 GMT
Content-Length: 43
